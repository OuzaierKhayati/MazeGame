// code with deleting the false path in the for of the function of correctDirection at line 253
...
function drawSquareBorder(x, y, size, fill, borderWidth = 0.5, radius = 0) {
    ctx.fillStyle = fill;
...
function drawSchema(height,draw,color){
    width=-30
    for (let i=0;i<50;i++){
        width+=30
        draw(width,height,30, color);
    }
}
...
function renderGame() {
    ...
    for(let i=0; i<31; i++){
        height+=30
        if (randomG || maze){drawSchema(height,drawSquare, 'black');}
        else {drawSchema(height,drawSquareBorder, 'white');}
    }

    for (let i=0; i<state.length; i++){
        if (maze){drawSquareBorder(state[i].x, state[i].y, 30,'white');}
        else {drawSquare(state[i].x, state[i].y, 30,'black');}
    }
    ...
    if (path.length>0 && maze ){
        if (!pathfinder){
            for (let i=1; i<path.length-1; i++){
                drawSquareBorder(path[i].x, path[i].y, 30,'yellow');
            }
        }
        else {
            for (let i=1; i<path.length-1; i++){
                drawSquareBorder(path[i].x, path[i].y, 30,'blue');
            }
        }
    }
...
function main() {
    ...
    document.addEventListener('keyup', (e) => {
        switch(e.key){
            case 'c': state.length=0; path.length=0; maze=false; pathfinder=false; randomG=false; k=undefined; mouseX = undefined; mouseY = undefined; startLocation=undefined; finalLocation=undefined; break; 
...
if (path.length>0){drawCorrectPath();}
...
function drawCorrectPath(){
    correctPath.push(path[path.length-1])
